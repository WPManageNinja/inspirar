        ...
        <div>
            <ul class="nav pull-right">
				<?php if ( is_user_logged_in() ): ?>
                    <li>
                        <a href="<?php echo esc_url( admin_url( 'nav-menus.php' ) ); ?>"><?php echo esc_html__( 'Create A Menu',
								'inspirar' ); ?></a>
                    </li>
				<?php else: ?>
                    <li>
                        <a href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php echo esc_html__( 'Home',
								'inspirar' ); ?></a>
                    </li>
				<?php endif; ?>
            </ul>
...
if ( ! function_exists( 'inspirar_logo' ) ) {
	function inspirar_logo() {
		...
		if ( ( is_home() && is_front_page() ) && has_custom_logo() ) {
			...
			echo '<a href="' . esc_url( home_url( '/' ) ) . '" rel="home"><img src="'
			     . esc_url( $inspirar_sub_page_logo ) . '"></a>';
		} else {
			echo '<h1 class="site-title"><a href="' . esc_url( home_url( '/' ) ) . '" rel="home">'
			     . esc_html( get_bloginfo( 'name' ) ) . '</a></h1>';
...
if ( ! function_exists( 'inspirar_page_banner' ) ) {
	function inspirar_page_banner() {
		$inspirar_page_breadcrumbs_section_visiblity = get_theme_mod( 'inspirar_page_breadcrumbs_section_visiblity',
			true );
		if ( has_post_thumbnail() ) {
			$inspirar_page_banner_bg_image = get_the_post_thumbnail_url( null, 'full' );
		}
		$inspirar_page_banner_bg_image = ( ! empty( $inspirar_page_banner_bg_image ) ) ? 'style="background-image: url('
		                                                                                 . esc_url( $inspirar_page_banner_bg_image )
		                                                                                 . ')"' : '';
		if ( ! is_front_page() ) {
			?>
                ...
                <div class="inspirar-page-banner-content d-table-cell align-bottom">
                    <div class="container">
                        <div class="row">
                            ...
                            <div class="col-sm-6 col-xs-12">
								<?php if ( $inspirar_page_breadcrumbs_section_visiblity ) {
									echo inspirar_breadcrumbs();
								} ?>
                            </div>
                        </div>
                    </div>
	...
	function inspirar_blog_header_title() {
		if ( is_home() && ! is_front_page() ) {
			...
			if ( is_day() ) :
				...
				printf( esc_html__( 'Daily Archives: %s', 'inspirar' ),
					'<span>' . esc_html( get_the_date() ) . '</span>' );
            elseif ( is_month() ) :
				/* translators: %s get theme monthly arcives date. */
				printf( esc_html__( 'Monthly Archives: %s', 'inspirar' ),
					'<span>' . esc_html( get_the_date( _x( 'F Y', 'monthly archives date format', 'inspirar' ) ) )
					. '</span>' );
            elseif ( is_year() ) :
				/* translators: %s get theme yearly arcives date. */
				printf( esc_html__( 'Yearly Archives: %s', 'inspirar' ),
					'<span>' . esc_html( get_the_date( _x( 'Y', 'yearly archives date format', 'inspirar' ) ) )
					. '</span>' );
			else :
			...
			printf( esc_html__( 'Search Results for: &ldquo;%s&rdquo;', 'inspirar' ),
...
if ( ! function_exists( 'inspirar_blog_banner' ) ) {
	function inspirar_blog_banner() {
		$inspirar_page_breadcrumbs_section_visiblity = get_theme_mod( 'inspirar_page_breadcrumbs_section_visiblity',
			true );
		?>
        <div class="inspirar-page-banner d-table overlay" <?php if ( get_header_image()
		                                                             !== ''
		) { ?> style="background-image: url('<?php esc_attr( header_image() ); ?>');" <?php } ?>>
                ...
                <div class="inspirar-page-banner-content d-table-cell align-bottom">
                    <div class="container">
                        <div class="row">
                            <div class="col-sm-6 col-xs-12">
                                <div class="inspirar-header-title">
                                    <h1><?php if ( function_exists( 'inspirar_blog_header_title' ) ) {
											inspirar_blog_header_title();
										} ?></h1>
                                </div>
                            </div>
                            <div class="col-sm-6 col-xs-12">
